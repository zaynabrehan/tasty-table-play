...
CREATE POLICY "Admins can view pending admin emails" ON public.pending_admin_emails FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can insert pending admin emails" ON public.pending_admin_emails FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Admins can delete pending admin emails" ON public.pending_admin_emails FOR DELETE TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

-- Function to auto-grant admin on login if email is in pending list
CREATE OR REPLACE FUNCTION public.check_and_grant_pending_admin(_user_id uuid, _email text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM public.pending_admin_emails WHERE lower(email) = lower(_email)
  ) INTO _found;
  ...
  IF _found THEN
    ...
    DELETE FROM public.pending_admin_emails WHERE lower(email) = lower(_email);
    RETURN true;
  END IF;
